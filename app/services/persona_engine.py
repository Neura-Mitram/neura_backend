...
async def run_persona_engine(db: Session, user: User) -> dict:
    ...
    try:
        ...
        if recent_moods:
            ...
            top_mood = sorted(mood_freq.items(), key=lambda x: x[1], reverse=True)[0][0]
            traits["recent_emotion"] = top_mood
...
def analyze_usage_pattern(db: Session, user: User) -> str:
    ...
    journals = db.query(JournalEntry).filter(JournalEntry.user_id == user.id, JournalEntry.timestamp >= week_ago).count()
    checkins = db.query(DailyCheckin).filter(DailyCheckin.user_id == user.id, DailyCheckin.timestamp >= week_ago).count()

    info_msgs = db.query(Message).filter(
