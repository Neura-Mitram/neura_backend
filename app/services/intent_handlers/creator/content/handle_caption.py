...
from app.utils.prompt_templates import caption_prompt
...
async def handle_creator_caption(request: Request, user: User, message: str, db: Session):
    ...
    # 🔍 Detect emotion
    emotion_label = await update_emotion_status(user, message, db, source="creator_caption")
    ...
    try:
        ...
        db.commit()
        track_usage_event(db, user, category="creator_caption")
        return {
            "message": "✅ Captions generated.",
            "captions": response.strip().split("\n"),
